...
public class TestRoll3DView extends View {
    ...
    public TestRoll3DView(Context context) {
        this(context,null);
    }

    public TestRoll3DView(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }
    public TestRoll3DView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        LogUtil.INSTANCE.e("Roll3DView");
        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.Roll3DView);
        int model = typeArray.getInt(R.styleable.Roll3DView_rollmodel,0);
        switch (model){
            case 0:
                setRollMode(RollMode.Roll2D);
                break;
            case 1:
                setRollMode(RollMode.Whole3D);
                break;
            case 2:
                setRollMode(RollMode.SepartConbine);
                break;
            case 3:
                setRollMode(RollMode.RollInTurn);
                break;
            case 4:
                setRollMode(RollMode.Jalousie);
                break;
            default:
        }
        typeArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null == bitmapList || bitmapList.size() <= 0)
            return;
        LogUtil.INSTANCE.e("onDraw_"+rollMode);
        switch (rollMode) {
            case Roll2D:
